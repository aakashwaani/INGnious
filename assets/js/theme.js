  ...
  var e = {
    init: () => {
      e.stickyHeader(),
        ...
        e.headerButtons(),
        e.isotope(),
        e.onepageHeaderOffset(),
        e.onepageNavLinks(),
        e.anchorSmoothScroll(),
        e.svgInject(),
        e.backgroundImage(),
        e.backgroundImageMobile(),
        e.imageHoverOverlay(),
        e.rellax(),
        e.scrollCue(),
        e.showMoreItems(),
    ...
    stickyHeader: () => {
      s(".navbar").length &&
        new Headhesive(".navbar", {
          ...
          onStick: function () {
            s(s.SmartMenus.Bootstrap.init),
              s(
                ".navbar:not(.fixed) .language-select .dropdown-menu"
              ).removeClass("show");
          },
          onUnstick: function () {
            s(".navbar.fixed .language-select .dropdown-menu").removeClass(
              "show"
            );
          },
        });
    ...
    dropdownAnimation: () => {
      s(".navbar .navbar-nav:not(.navbar-nav-other)")
        .bind({
        ...
        .on(
          ...
          "ul",
          function (t) {
            s(this).removeClass("show-animation hide-animation"),
              t.stopPropagation();
          }
        );
    },
    ...
    anchorSmoothScroll: () => {
      s(function () {
        function e(t) {
          (t = t.length ? t : s("[name=" + this.hash.slice(1) + "]")).length &&
            s("html,body").animate(
        ...
        setTimeout(function () {
          ...
          s('a.scroll[href*="#"]:not([href="#"])').on("click", function () {
            if (
              location.pathname.replace(/^\//, "") ==
                this.pathname.replace(/^\//, "") &&
              location.hostname == this.hostname
            )
              return e(s(this.hash)), !1;
          });
      });
    },
    svgInject: () => {
    ...
    backgroundImageMobile: () => {
      ...
      ) || s(".image-wrapper").addClass("mobile");
    },
    imageHoverOverlay: () => {
      s(".overlay:not(.caption) > a, .overlay:not(.caption) > span").prepend(
        '<span class="bg"></span>'
      );
    },
    rellax: () => {
    ...
    bsModal: () => {
      var t,
        e = window.innerWidth - s("body").innerWidth();
      document.querySelectorAll(".modal").forEach((t) => {
        t.addEventListener("show.bs.modal", function (t) {
          s(".navbar.fixed").css("padding-right", e),
            s(".progress-wrap").css("margin-right", e);
        }),
          t.addEventListener("hidden.bs.modal", function (t) {
            s(".navbar.fixed").css("padding-right", ""),
              s(".progress-wrap").css("margin-right", "");
          });
      }),
        0 < s(".modal-popup").length &&
          ((t = new bootstrap.Modal(document.querySelector(".modal-popup"))),
                  ...
                  !0 === e.checkValidity() &&
                    ...
                    s.ajax({
                      ...
                      success: function (t) {
                        ...
                        e &&
                          a &&
                          (s(".contact-form").find(".messages").html(t),
                          s(".contact-form")[0].reset());
                      },
                    }));
              },
              !1
    ...
    pricingSwitcher: () => {
      s(".pricing-wrapper").each(function (t, e) {
        ...
        a.find(".pricing-switcher").on("click", function () {
          a.find(".pricing-switcher").toggleClass("pricing-switcher-active"),
            a.find(".price").removeClass("price-hidden"),
            a.find(".price").toggleClass("price-show price-hide");
        });
      });
    },
    textRotator: () => {
      null != document.querySelector(".rotator-zoom") &&
        ...
        null != document.querySelector(".rotator-fade") &&
